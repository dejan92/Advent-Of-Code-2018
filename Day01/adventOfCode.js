...
arrOfNums = stringOfNumbers.split(" ");
...
while (!shouldStop) {
  for (let index = 0; index < arrOfNums.length; index++) {
    const currentNumber = arrOfNums[index];

    newFrequency = newFrequency + Number(currentNumber);

    if (frequency.includes(newFrequency)) {
      shouldStop = true;
      console.log(newFrequency);
      break;
    }

    frequency.push(newFrequency);
  }
}
